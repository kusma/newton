...
static INLINE u32 urcp(u32 i)
{
	u32 zero_bits = clz(i);
	u32 v = i << zero_bits;
	u32 v1 = rcp_table[(v >> 23)];
	u32 v2 = (v >> 14) & 511;
	u32 v3 = mul16x32(v1, v2) + v1; // MLA in ARM :)
	u32 v4 = v3 >> (31 - zero_bits);
	return v4;
}

static INLINE s32 rcp(s32 i)
{
	int neg = i < 0;
	if (neg) i = -i;
	int v = (s32)urcp((u32)i);
	if (neg) return -v;
	return v;
}
...
extern const u16 IntRcp_Table[160 * 2];
