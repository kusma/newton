...
//					heap.push_back( (unsigned long) loop );
	...
	void rleImage(vector<unsigned short>& dst, const u8* src, const u8* src_end, int bpp, bool reserveMSB) {
		...
		int altall=0;
		while (src!=src_end) {
			int cmd = *src;// >> (8-bpp); assumed grayscale. bpp must be restricted earlier.
			// check for bpp out of range
			{
				static int yo=0;
				...
				if (cmd & (~mask) && !yo) {
					yo=1;
					fprintf(stderr, "Image index out of legal range when compressing!\n");
				}
				cmd &= mask;
			}
	...
	void updateList(vector<unsigned short>& dst, vector<unsigned short>& prev, vector<unsigned short>& curr) {
		...
		FORI(prev.size()) {
			if (i>0 && i<(int)prev.size()-1 && prev[i-1]==prev[i] && prev[i+1]==prev[i]) continue;
			locations_prev.insert( multimap<int, int>::value_type(prev[i], i));     
		}
		FORI(curr.size()) {
			if (i>0 && i<(int)curr.size()-1 && curr[i-1]==curr[i] && curr[i+1]==curr[i]) continue;
			...
			for (int direction_iter=-1; abs(direction_iter)==1; direction_iter+=2) {
				...
				for (; matches_iter != matches_prev.second; ++matches_iter) {
					...
					while (1) {
						...
						if (previ>=(int)prev.size()) break;
						if (curri>=(int)curr.size()) break;
